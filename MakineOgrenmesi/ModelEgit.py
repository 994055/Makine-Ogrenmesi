...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def confusionMatrix(self):
        if len(self.confision)!=0:
            df_cm = pd.DataFrame(self.confision, index=[i for i in "10"],
                                 columns=[i for i in "10"])
            plt.figure(figsize=(10, 7))
            sn.heatmap(df_cm, annot=True,fmt='d')
            plt.show()
        else:
            QMessageBox.about(self, "Hata", "Önce model eğitimini gerçekleştirmeniz gerek!")
    ...
    def confusionMatrixKFold1(self):
        if len(self.confisionFold1) != 0:
            df_cm = pd.DataFrame(self.confisionFold1, index=[i for i in "10"],
                                 columns=[i for i in "10"])
            plt.figure(figsize=(10, 7))
            sn.heatmap(df_cm, annot=True,fmt='d')
            plt.show()
    ...
    def confusionMatrixKFold2(self):
        if len(self.confisionFold2) != 0:
            df_cm = pd.DataFrame(self.confisionFold2, index=[i for i in "10"],
                                 columns=[i for i in "10"])
            plt.figure(figsize=(10, 7))
            sn.heatmap(df_cm, annot=True,fmt='d')
            plt.show()
    ...
    def confusionMatrixKFold3(self):
        if len(self.confisionFold3) != 0:
            df_cm = pd.DataFrame(self.confisionFold3, index=[i for i in "10"],
                                 columns=[i for i in "10"])
            plt.figure(figsize=(10, 7))
            sn.heatmap(df_cm, annot=True,fmt='d')
            plt.show()
    ...
    def confusionMatrixKFold4(self):
        if len(self.confisionFold4) != 0:
            df_cm = pd.DataFrame(self.confisionFold4, index=[i for i in "10"],
                                 columns=[i for i in "10"])
            plt.figure(figsize=(10, 7))
            sn.heatmap(df_cm, annot=True,fmt='d')
            plt.show()
    ...
    def veriSetiSec(self):
        dosya_yolu = QFileDialog.getOpenFileName(parent=self, caption="Dosya seç",directory="C:\\Users\\kalbi\\OneDrive\\Masaüstü\\MakineOgrenmesi\\Dataset")
    ...
    def readData(self,dosyaAdi):
        anaKlasor = "./"
        data = pd.read_csv(anaKlasor + "Dataset/" + dosyaAdi)
        data.info()
        labels=data.columns
        Xveri=data.values
        ...
        for i, data in enumerate(Xveri):
            ...
            self.tblY.setItem(i, 0, QTableWidgetItem(str(y[i])))
    ...
    def ModelTahmin(self):
        try:
            ...
            yas=int(self.leYas.text())
            cinsiyet=0
            if self.rbErkek.isChecked()==True:
                cinsiyet=0
            elif self.rbKadin.isChecked()==True:
                cinsiyet=1
            gogusAgri=int(self.getIndexAgri())
            dinlenmeKanBasinci=float(self.leDinlenmeKanBasinci.text())
            kolestrol=int(self.leKolestrol.text())
            kanSekeri=0
            if self.rbESekerEvet.isChecked()==True:
                kanSekeri=0
            elif self.rbSekeHayir.isChecked()==True:
                kanSekeri=1
            kalpECG=self.getIndexkalpECG()
            maxNabiz=float(self.leMaxNabiz.text())
            gogusEgzersiz=0
            if self.rbGogusAgriEvet.isChecked()==True:
                gogusEgzersiz=0
            elif self.rbGogusAgriHayir.isChecked()==True:
                gogusEgzersiz=1
            Oldpeak=float(self.leOldpeak.text())
            stEgimi=self.getIndexSTegimi()
            tahmin=[]
            tahmin.append(yas)
            tahmin.append(cinsiyet)
            tahmin.append(gogusAgri)
            tahmin.append(dinlenmeKanBasinci)
            tahmin.append(kolestrol)
            tahmin.append(kanSekeri)
            tahmin.append(kalpECG)
            tahmin.append(maxNabiz)
            tahmin.append(gogusEgzersiz)
            tahmin.append(Oldpeak)
            tahmin.append(stEgimi)
            tahmin=np.array(tahmin)
            sonDizi=[]
            sonDizi.append(tahmin)
            sonuc = model.predict(sonDizi)
            if sonuc == 0:
                self.lblSonuc.setText("Kişi hasta değil")
                self.lblSonuc.setStyleSheet("background-color: rgb(0, 0, 255);")
            elif sonuc == 1:
                self.lblSonuc.setText("Kişi hasta!")
                self.lblSonuc.setStyleSheet("background-color: rgb(255, 0, 0);")
        except:
